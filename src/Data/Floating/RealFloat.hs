...
import qualified Data.Floating.Ryu.Common as R
import qualified Data.Floating.Ryu.F2S as RF
import qualified Data.Floating.Ryu.D2S as RD
...
formatFloat fmt prec f =
  case fmt of
    ...
    FFExponent -> RF.f2sScientific' f
    FFFixed -> d2Fixed (realToFrac f) prec

{-# INLINABLE formatDouble #-}
formatDouble :: FFFormat -> Maybe Int -> Double -> BB.Builder
...
d2Fixed :: Double -> Maybe Int -> BB.Builder
d2Fixed f prec =
  case specialStr f of
    Just b -> b
...
-- | Char7 encode a 'Char'.
{-# INLINE char7 #-}
char7 :: Char -> BB.Builder
char7 = BP.primFixed BP.char7

-- | Char7 encode a 'String'.
{-# INLINE string7 #-}
string7 :: String -> BB.Builder
string7 = BP.primMapListFixed BP.char7

sign :: RealFloat a => a -> BB.Builder
sign f = if f < 0 then char7 '-' else mempty

specialStr :: RealFloat a => a -> Maybe BB.Builder
specialStr f
...
showFixed :: Word64 -> Int -> Maybe Int -> BB.Builder
showFixed m e prec =
  case prec of
   Nothing
     | e <= 0 -> char7 '0'
